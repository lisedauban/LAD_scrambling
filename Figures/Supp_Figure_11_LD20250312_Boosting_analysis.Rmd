---
title: "Supp_Figure_11_LD20250312_Boosting_analysis"
date: "2023-11-28"
output: 
  html_document:
...
# Figure Supp 11A - TF in Boosting regions
```{r}
...
# Figure Supp 11C - rmes fold change
## LAD1
```{r}
# Load data
...
# Figure Supp 11F - repeats sub-fammilies
## combine LAD1
```{r}
...
# Figure Supp 11H - Secondary structures
## LAD1
### load data
```{r}
...
### plot
```{r}
all_motifs %>% group_by(sample, Type) %>% summarise(n = n()) %>% mutate(norm = ifelse(sample == "LAD1", n, n*LAD1_length/R45_length)) %>% pivot_wider(names_from = sample, values_from = c(n, norm)) %>% mutate(FC = norm_R45 / n_LAD1) %>% ggplot(aes(x=reorder(Type,log2(FC)), y=log2(FC))) + 
    geom_col() + 
    geom_hline(yintercept = c(1, -1), linetype = "dashed", alpha = 0.5) +
    labs(title = "",
         x = "",
...
all_motifs %>% group_by(sample, Type) %>% summarise(n = n()) %>% mutate(norm = ifelse(sample == "LAD2", n, n*LAD2_length/LAD2nr_length)) %>% pivot_wider(names_from = sample, values_from = c(n, norm)) %>% mutate(FC = norm_LAD2nr / n_LAD2) %>% ggplot(aes(x=reorder(Type,log2(FC)), y=log2(FC))) + 
    geom_col() + 
    geom_hline(yintercept = c(1, -1), linetype = "dashed", alpha = 0.5) +
    labs(title = "",
         x = "",
...
# Figure Supp 11I - H3K9me2 and H3K9me3
...
H3K9me3.bw2 <- import("/DATA/usr/l.dauban/projects/pA-DamID/E2063_rec_clones_T3_T201h49_T201h143_2_htchm/results_mm10_nextseq_strainspecific/tracks/normalized/bin-25kb/T201h49_H3K9me3-25kb-combined.bw")
...
H3K9me3.bw2 %>% data.frame() %>% filter(seqnames == "chr9" & region != "other") %>% mutate(region = factor(region, levels = c("other", "R45", "LAD1", "LAD2.2", "LAD2.1"))) %>%
  ...
  geom_beeswarm(alpha = 0.5, show.legend = FALSE) +
  geom_signif(test = "wilcox.test", test.args = list(alternative = c("two.sided")), comparisons = list(c("R45", "LAD1"), c("LAD2.2", "LAD2.1")), color = "black", y_position = c(2.3, 2.3), show.legend = FALSE) +
...
ggsave("FigS11I_LD20241125_H3K9me3.pdf", path = "./output/", height = 5, width = 5)
