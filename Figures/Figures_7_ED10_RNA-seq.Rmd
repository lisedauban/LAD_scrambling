---
title: "Figures_7_ED10_LD20231128_RNA-seq"
date: "2023-11-28"
output: 
  html_document:
...
This is Figure 7 and Extended Figure 10
...
# Figure 7A - LaminB1 changes in d3
```{r}
# Import data
...
# # save
# ggsave("Fig7A_LD20230815_T201h65_LB1_diff.pdf", path = "./output/", height = 5, width = 20)


```


# Figure 7B - LaminB1 changes in d6
```{r}
# Import data
...
# # save
# ggsave("Fig7B_LD20230815_T48h20_LB1_diff.pdf", path = "./output/", height = 5, width = 20)


```


# Figure 7C - LaminB1 changes in i7
```{r}
# Import data
...
# # save
# ggsave("Fig7C_LD20230815_T201h49_LB1_diff.pdf", path = "./output/", height = 5, width = 20)
...
# ggsave("Fig7D_LD20230717_d3.pdf", path = "./output/", height = 9, width = 13)
...
# ggsave("Fig7E_LD20230717_d6.pdf", path = "./output/", height = 9, width = 13)
...
# ggsave("Fig7F_LD20230717_i7.pdf", path = "./output/", height = 9, width = 13)

```


# Extended Data 10A-B - LaminB1 changes in i7 - clone r7
```{r}
# Import data
T201h49r7 = rtracklayer::import("/DATA/usr/l.dauban/projects/pA-DamID/E2063_rec_clones_T3_T201h49_T201h143_1_LB1_K9me2/2nd_round/Differential_tracks/tib_T201h49r7.bw")

# Invert
T201h49r7[seqnames(T201h49r7) == "chr9" & start(T201h49r7) > clones.df[1,2] & end(T201h49r7)< clones.df[4,2]]$score <- rev(T201h49r7[seqnames(T201h49r7) == "chr9" & start(T201h49r7) > clones.df[1,2] & end(T201h49r7)< clones.df[4,2]]$score)

T201h49r7 <- T201h49r7 %>% as.tibble() %>% dplyr::rename(T201h49_ = score)
...
# Plot
PlotDataTracksLight(input_tib = T201h49r7,
                    exp_names = c("T201h49_"),
                    color_groups = c(T201h49_ = "T201h49_"),
                    plot_chr = "chr9", plot_start = 109937608, plot_end = 114948158,
                    color_list = c("#D6604D", "#4393C3"),
...
# save
# ggsave("SuppFig14AB_LD20250407_T201h49_LB1_diff.pdf", path = "./output/", height = 5, width = 20)

```



# Extended Data 10C - i12 
```{r}
# load count table
...
# ggsave("SuppFig14C_LD20230717_i12.pdf", path = "./output/", height = 9, width = 13)


```


# Extended Data 10C - i10
```{r}
...
# ggsave("SuppFig14C_LD20230717_i10.pdf", path = "./output/", height = 9, width = 13)

```


# Figure 7G - scatter plots
```{r}
...
RNA.damID.table.all.all %>% filter(start != 'NA' & K9me3_diff != 'NA') %>% filter(start != 'NA' & LB1_diff != 'NA') %>% select(Par, Par_tot, Par_corr, log2, LB1_diff, K9me3_diff, sample, allele, gene_name, pvalue, sig) %>% gather(key, value, -Par, -Par_tot, -Par_corr, -log2, -sample, -allele, -gene_name, -pvalue, -sig) %>% group_by(gene_name) %>% filter(Par_corr > 5) %>% filter(allele == "CAST") %>%
  ggplot() +
  geom_point(aes(x=value, y=log2, color = log10(Par/Par_tot * 1e6), label = paste(gene_name,",",sample), stroke = 0), size = 3) +
...
# ggsave("Fig7G_LD20231219_scatter_plots.pdf", path = "./output/", height = 4, width = 8)

```


# Figure 7H - Linear model
```{r}
...
# ggsave("Fig7H_LD20230717_linear_model.pdf", path = "./output/", height = 3, width = 3)
